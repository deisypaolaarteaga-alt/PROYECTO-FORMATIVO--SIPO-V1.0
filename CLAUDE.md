...
### Applied migrations (53 total, in order)
...
| `20260603100000_portal_cliente.sql` | **Portal del Cliente Fase 1**: tabla `presupuesto_tokens` (token 32-bytes hex, expires_at, cliente_email, visto_count, cliente_accion CHECK); `check_budget_estado` extendida con 5 nuevos estados (`enviado_a_cliente`, `visto_por_cliente`, `aprobado_por_cliente`, `rechazado_por_cliente`, `con_observaciones`); RLS: SELECT público (anon+authenticated), INSERT/UPDATE service_role. | ✅ applied |
| `20260603200000_clientes_unique.sql` | **Unicidad clientes**: Limpieza de duplicados por `nit_cedula`, `email`, `nombre_razon_social` (conserva menor id por user_id); 3 índices únicos parciales `WHERE activo = true`. | ✅ applied |
| `20260603210000_clientes_drop_nombre_unique.sql` | **Corrección**: Elimina `clientes_nombre_unique` — en Colombia dos clientes pueden tener igual `nombre_razon_social` con diferente NIT. Solo `nit_cedula` y `email` mantienen unicidad. | ✅ applied |
...
| `presupuesto_tokens` | `token TEXT UNIQUE` (hex 32 bytes), `budget_id UUID`, `expires_at TIMESTAMPTZ`, `visto_at TIMESTAMPTZ`, `visto_count INT DEFAULT 0`, `cliente_accion TEXT CHECK (IN 'aprobado'\|'rechazado'\|'observaciones')`, `cliente_comentario TEXT`, `cliente_firma_nombre TEXT`. RLS: SELECT público (anon+authenticated), INSERT/UPDATE solo service_role. |
...
| `src/app/presupuesto-publico/[token]/page.tsx` | Ruta pública sin auth — Next.js dynamic route; llama `getPresupuestoPublico(token)` y registra la vista vía `registrarVistaToken`. Excluida del middleware. |
| `src/app/presupuesto-publico/[token]/PortalClientePage.tsx` | Client Component del portal: resumen financiero (CD, AIU, IVA, Total Oferta), tabla de capítulos colapsable con % CD, datos de empresa constructora + datos del cliente, formulario de respuesta (Aprobar/Rechazar/Comentar) con campo firma nombre. |
...
| `src/lib/email/brevo.ts` | Helper de envío de emails con `@getbrevo/brevo` v5 — `enviarEmailPortalCliente(to, subject, html)`. Usado por `enviarPresupuestoAlCliente` para el email HTML responsive con enlace al portal. |
...
> **Snapshot:** 2026-06-03 (actualizado) — `tsc --noEmit --skipLibCheck` limpio (0 errores). `pnpm build` exitoso. **54 migraciones en `migrate.js`** (todas rastreadas). Rama `rama-deisy`. **18 archivos de Server Actions** (`catalogo.ts` con 17 exports; `auth.ts` con 9 exports; `plantillas.ts` con 7 exports; `portal-cliente.ts` con 5 exports; `proyectos.ts` ampliado con 5 nuevas acciones: `archivarProyecto`, `eliminarProyecto`, `desArchivarProyecto`, `archivarPresupuesto`, `eliminarPresupuesto`). 5 componentes PDF. Motor `motor-presupuesto.ts` (219 líneas) con **9 tests Vitest pasando** (110 líneas). `src/middleware.ts` activo (excluye `/presupuesto-publico/**`). Sistema de roles `usuario`/`super_admin` con RLS. Módulo Catálogo editable para `super_admin`. Login 2FA por OTP. `fn_duplicar_presupuesto` atómica. hCaptcha eliminado. **35 tablas + 5 vistas en BD** (`presupuesto_tokens` añadida). Filtros de período. Módulo Plantillas personales completo. Portal del Cliente completo (ruta pública `/presupuesto-publico/[token]`, email Brevo, botón "Enviar al cliente" en editor). `proveedores.activo` + `activities.catalogo_actividad_id` añadidos. Flujo completo eliminar/archivar proyectos y presupuestos con ConfirmDialog.
...
~~- **tipo_obra en plantillas**~~ ✅ 2026-06-02 — `guardarComoPlantilla` recibe `tipoObraParam?: string|null` + fallback admin-client via `projects.tipo_obra`. `ModalGuardarPlantilla` recibe prop `tipoObra` y lo pasa a la acción. `EditorPresupuesto` pasa `budget.projects?.tipo_obra`. `PlantillasClient` añade "Cambiar tipo de obra" en menú ⋯ con mini-modal de 6 opciones + `actualizarTipoObraPlantilla` server action. `ModalNuevoPresupuesto` agrupa plantillas en "Recomendadas" / "Otras plantillas" cuando hay contexto de tipo_obra. `tsc` limpio ✅.
~~- **Plantillas personales de usuario**~~ ✅ 2026-06-01 — `user_plantillas` + 3 tablas hijas con RLS (migraciones `20260601200000` + `20260601400000`). `src/actions/plantillas.ts` (8 exports: `guardarComoPlantilla`, `getMisPlantillas`, `eliminarPlantilla`, `aplicarPlantilla`, `renombrarPlantilla`, `getDetallePlantilla`, `actualizarEstructuraPlantilla`, `actualizarTipoObraPlantilla`). `ModalGuardarPlantilla.tsx` en header del editor. `ModalNuevoPresupuesto` con sección "Mis plantillas" + 3 modos (estructura/con precios/todo igual). Página `/plantillas` con `PlantillasClient.tsx`, `DrawerDetallePlantilla.tsx` (preview completo de capítulos+actividades+APU items), `ModalRenombrarPlantilla.tsx`. `tsc` limpio ✅.
...
### Completado ✅
- ~~**Portal del Cliente completo**~~ ✅ 2026-06-03 — Fase 1: migración `20260603100000_portal_cliente.sql`: tabla `presupuesto_tokens` (token hex 32 bytes, expires_at, visto_at, visto_count, cliente_accion, cliente_comentario, cliente_firma_nombre) + 5 nuevos estados en `budgets` (`enviado_a_cliente`, `visto_por_cliente`, `aprobado_por_cliente`, `rechazado_por_cliente`, `con_observaciones`) + RLS público para tokens. `portal-cliente.ts` (5 exports: `enviarPresupuestoAlCliente`, `registrarVistaToken`, `responderPresupuesto`, `getTokenInfo`, `getPresupuestoPublico`). Fase 2: página pública `/presupuesto-publico/[token]` sin login (`page.tsx` + `PortalClientePage.tsx` client component) con resumen financiero, tabla de capítulos colapsable con % CD, datos empresa constructora + cliente, formulario de respuesta (Aprobar/Rechazar/Comentar + campo firma nombre). Email automático via `src/lib/email/brevo.ts` (`@getbrevo/brevo` v5) con HTML responsive. `ModalEnviarCliente.tsx` en editor con botón "Enviar al cliente", indicador de estado y banners de notificación. Middleware actualizado para excluir `/presupuesto-publico/**`. `tsc` limpio ✅ `pnpm build` ✅.
