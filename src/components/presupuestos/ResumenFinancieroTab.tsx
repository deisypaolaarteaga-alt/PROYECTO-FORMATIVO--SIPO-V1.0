...
export function ResumenFinancieroTab({ budget, subtotalDirecto }: ResumenFinancieroTabProps) {
  ...
  const fmt = (d: Decimal | number) =>
    formatearCOP(typeof d === 'number' ? d : d.toDecimalPlaces(0).toNumber());

  const aiuBase = adminPct + imprevPct + utilidadPct || 1;
      ...
      <div className="bg-steel-dark text-white p-6 rounded-2xl flex items-start gap-4">
        ...
        <div className="space-y-1">
          ...
          <p className="text-[11px] text-steel-light leading-relaxed">
            El valor neto estimado de{' '}
            <strong className="text-white">{fmt(neto)}</strong> contempla retenciones de
            ReteFuente ({reteFuentePct}%) e ICA ({icaPct}%).
            Recuerda que estas retenciones son informativas para el flujo de caja y no afectan
            el valor contractual del presupuesto.
          </p>
        </div>
      </div>
    </div>
...
function LegendItem({ label, value, pct, color }: { label: string; value: string; pct: number; color: string }) {
  return (
    <div className="space-y-1">
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="h-2 bg-sand rounded-full overflow-hidden">
        <motion.div
          initial={{ width: 0 }}
          animate={{ width: `${(pct / base) * 100}%` }}
          className={cn('h-full', barClass)}
        />
      </div>
      <p className={cn('text-right text-[11px] font-mono text-stone', valueClass)}>{value}</p>
    </div>
  );
}
